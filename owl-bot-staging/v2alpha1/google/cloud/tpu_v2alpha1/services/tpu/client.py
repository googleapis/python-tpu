...
class TpuClient(metaclass=TpuClientMeta):
    ...
    @staticmethod
    def queued_resource_path(project: str,location: str,queued_resource: str,) -> str:
        """Returns a fully-qualified queued_resource string."""
        return "projects/{project}/locations/{location}/queuedResources/{queued_resource}".format(project=project, location=location, queued_resource=queued_resource, )

    @staticmethod
    def parse_queued_resource_path(path: str) -> Dict[str,str]:
        """Parses a queued_resource path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/queuedResources/(?P<queued_resource>.+?)$", path)
        return m.groupdict() if m else {}
    ...
    def create_queued_resource(self,
    ...
    def delete_queued_resource(self,
