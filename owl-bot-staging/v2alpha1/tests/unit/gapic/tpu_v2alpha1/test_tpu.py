...
async def test_list_queued_resources_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_queued_resources(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_get_queued_resource(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = cloud_tpu.QueuedResource(
            name='name_value',
            tpu=cloud_tpu.QueuedResource.Tpu(node_spec=[cloud_tpu.QueuedResource.Tpu.NodeSpec(parent='parent_value')]),
            best_effort=None,
        )
